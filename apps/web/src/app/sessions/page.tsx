...
export default function SessionsPage() {
  ...
  const loadSessions = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8787/api'}/users/sessions?page=${page}&limit=${pagination.limit}`, {
        method: 'GET',
        headers: {
  ...
  const confirmDeleteSession = useCallback(async () => {
    ...
    try {
      ...
      const token = await getAccessToken()
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8787/api'}/sessions/${sessionId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const confirmBulkDelete = useCallback(async () => {
    try {
      ...
      const token = await getAccessToken()
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8787/api'}/sessions/bulk`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
